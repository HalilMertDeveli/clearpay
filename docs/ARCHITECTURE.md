...
- **JSON API:** JWT + OpenAPI. `POST /api/transfers` + `Idempotency-Key` (201 / 409). TASK-06 landed. Q2 Flutter: `GET /api/wallet`, movements, receipts; `POST /api/topup` / `withdraw` (T-061). Live chrome (T-071): `/hubs/wallet` — cookie (Razor) veya JWT `access_token` query (Flutter). Payload yenileme ipucu; bakiye `GET /api/wallet`. Swagger UI `/swagger` (T-050). Harici client cookie’ye bağlanmaz.
